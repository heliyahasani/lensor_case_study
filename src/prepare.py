...
class DataPreparation:
    ...
    def merge_dataframes(self, images_df: pd.DataFrame, annotations_df: pd.DataFrame, categories_df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def get_balanced_samples(self, dataframe: pd.DataFrame, max_samples_per_category: int = 100) -> pd.DataFrame:
        ...
        balanced_df = dataframe.groupby('category_id').apply(lambda x: x.sample(min(len(x), max_samples_per_category))).reset_index(drop=True)
        return balanced_df
    ...
    def prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        validation_categories_df, validation_images_df, validation_annotations_df = self.load_json_to_dataframe(self.val_json)
        ...
        validation_merged_df = self.merge_dataframes(validation_images_df, validation_annotations_df, validation_categories_df)
